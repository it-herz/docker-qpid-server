...
RUN apt-get update && apt-get dist-upgrade -y && apt-get install -y uuid-dev clang libssl-dev libsasl2-dev wget cmake make ruby-dev doxygen ruby valgrind pkg-config libboost-all-dev libdb5.3++-dev libdb-dev libaio-dev swig libxqilla-dev libxerces-c-dev libpthread-stubs0-dev libnss3-dev libnspr4-dev python-dev graphviz help2man krb5-user libgssapi-krb5-2 && \
    apt-get clean && rm -rf /var/lib/apt/lists/* 

RUN cd /root && mkdir proton && cd proton && wget https://dist.apache.org/repos/dist/dev/qpid/proton/0.11.0-rc/qpid-proton-0.11.0-rc.tar.gz && tar xzvpf qpid-proton* && \
